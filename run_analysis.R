...
names(Dataset_Train_Combine) <- c(c('Subject', 'Activity'), Dataset_features)
...
#Giving names to the elements in the Train combined data set
names(Dataset_Test_Combine) <- c(c('Subject', 'Activity'), Dataset_features)
...
Data_activity_labels <- read.table('./UCI HAR Dataset/activity_labels.txt', header=FALSE)
